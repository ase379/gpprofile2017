...
:gpprof
  ...
  echo.
    if exist ..\BIN\gpprof.exe del ..\BIN\gpprof.exe >nul
    if exist GpProf.UI\gpprof.cfg del GpProf.UI\gpprof.cfg >nul
  :gpprof_brcc_baggage
    brcc32 -r GpProf.UI\baggage.rc >build.log
    if not errorlevel 1 goto gpprof_brcc
    goto error
  :gpprof_brcc
    brcc32 GpProf.UI\gpprof.rc >build.log
    if not errorlevel 1 goto gpprof_dcc
    type build.log | more
    goto error
  :gpprof_dcc
    DCC32.exe -Q -B GpProf.UI\gpprof.dpr >build.log
